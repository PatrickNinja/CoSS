...
class transformer(nn.Module):
    def __init__(self, config):
        ...
        search_method = getattr(config, 'decode_method', 'beam')
    ...
    def forward(self, **kwargs):
        ...
        src_outputs_graph = src_embed.view(batch_size, num_seq, seq_len, -1)
        sent_rep_graph = src_outputs_graph.mean(dim=-2)
        graph_outputs = self.graph_Encoder_f(sent_rep_graph, graph) + self.graph_Encoder_r(sent_rep_graph,
                                                                                           graph.transpose_(-1, -2))
        ...
        encoder_outputs = self.norm(self.dropout(F.relu6(encoder_outputs)))

        if kwargs['mode'] == 'train':
            ground_truth = kwargs['ground_truth']
            tgt_embed = self.tgt_embed(ground_truth)
            tgt_mask = triu_mask(ground_truth.size(1)).to(ground_truth.device) | pad_mask(ground_truth, self.PAD_index)
